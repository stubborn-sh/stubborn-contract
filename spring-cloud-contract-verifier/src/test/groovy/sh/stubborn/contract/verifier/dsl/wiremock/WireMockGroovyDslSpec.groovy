...
class WireMockGroovyDslSpec extends Specification implements WireMockStubVerifier {
	...
	def 'should convert groovy dsl stub to wireMock stub for the client side'() {
		given:
			sh.stubborn.contract.spec.Contract groovyDsl =
					sh.stubborn.contract.spec.Contract.
							make {
								request {
									method('GET')
...
'''), wireMockStub)
	...
	def "should not allow query parameter with a different absent variation for server/client"() {
		when:
			sh.stubborn.contract.spec.Contract.make dsl
		then:
			def e = thrown(IllegalStateException)
			...
			AssertionUtil.assertThatJsonsAreEqual(('''
{
  ...
  "response" : {
	...
	def 'should generate stub with multipart parameters'() {
		given:
			// tag::multipartdsl[]
			sh.stubborn.contract.spec.Contract contractDsl =
					sh.stubborn.contract.spec.Contract.
							make {
								request {
									method "PUT"
	...
	def "should work with array of arrays"() {
		given:
			sh.stubborn.contract.spec.Contract groovyDsl = Contract.make {
				request {
					method 'POST'
	...
	def "should create a stub for dot separated keys"() {
		given:
			Contract groovyDsl = sh.stubborn.contract.spec.Contract.make {
				request {
					method 'PUT'
					url '/fraudcheck'
					body([ // (4)
