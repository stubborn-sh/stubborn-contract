 ...
 * lines) and the {@code // when:} response chain (response head plus the
 * {@code .<method>(url)} call).
 *
 * <p>
 * Built by {@link RequestModelBuilder} only for the eligible subset of HTTP contracts (no
 * body, cookies, query params, multipart, async/delay or request templates); otherwise
 * the whole method body is captured verbatim from the legacy generator and this model is
 * {@code null}.
 *
