...
@test "announce-website: dispatches release-published with repo and tag" {
	GH_TOKEN=token run "$SCRIPT" stubborn-contract 0.1.2
	[ "$status" -eq 0 ]
...
@test "announce-website: a failed dispatch never fails the release" {
	GH_TOKEN=token GH_EXIT=1 run "$SCRIPT" stubborn-contract 0.1.2
	[ "$status" -eq 0 ]
...
@test "announce-website: requires a repo and a version" {
	...
	GH_TOKEN=token run "$SCRIPT" stubborn-contract
	[ "$status" -ne 0 ]
}
