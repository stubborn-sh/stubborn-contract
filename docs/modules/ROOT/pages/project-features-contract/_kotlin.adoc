...
You need to explicitly pass the `stubborn-contract-spec-kotlin` dependency to your project plugin setup.
...
<plugin>
    ...
    <dependencies>
        <dependency>
            <groupId>sh.stubborn</groupId>
            <artifactId>stubborn-contract-spec-kotlin</artifactId>
            <version>${spring-cloud-contract.version}</version>
        </dependency>
    </dependencies>
</plugin>
...
buildscript {
	...
	dependencies {
		classpath "sh.stubborn:stubborn-contract-gradle-plugin:$\{scContractVersion}"
	}
}
...
dependencies {
    ...
    // Remember to add this for the DSL support in the IDE and on the consumer side
    testImplementation "sh.stubborn:stubborn-contract-spec-kotlin"
...
Generally you would use its contract function as follows: `sh.stubborn.contract.spec.ContractDsl.contract { ... }`.
You can also provide an import to the `contract` function (`import sh.stubborn.contract.spec.ContractDsl.Companion.contract`) and then call `contract { ... }`.
