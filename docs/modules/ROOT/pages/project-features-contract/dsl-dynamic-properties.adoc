...
Stubborn Contract comes with a series of predefined regular expressions that you can
...
[source,groovy,indent=0]
----
include::{plugins_path}/stubborn-converters/src/test/groovy/org/springframework/cloud/contract/verifier/wiremock/DslToWireMockClientConverterSpec.groovy[tags=wiremock,indent=0]
----
...
https://handlebarsjs.com/[Handlebars] `{{{ }}}` notation with custom Stubborn Contract
...
import static sh.stubborn.contract.verifier.util.ContractVerifierUtil.map;
...
to 2.5.1. The Stubborn Contract Verifier uses WireMock's
...
Currently, Stubborn Contract Verifier supports only JSON path-based matchers with the
...
[source,json,indent=0]
----
include::{plugins_path}/stubborn-converters/src/test/groovy/org/springframework/cloud/contract/verifier/wiremock/DslToWireMockClientConverterSpec.groovy[tags=matchers,indent=0]
----
