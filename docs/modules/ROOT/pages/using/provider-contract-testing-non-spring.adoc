...
You can read xref:getting-started/first-application.adoc[Developing Your First Stubborn Contract-based Application] to see the flow for provider contract testing with stubs in Nexus or Artifactory.
...
By default, the Stubborn Contract Plugin uses Rest Assured's `MockMvc` setup for the
...
* We have set up the Stubborn Contract plugin to use the `EXPLICIT` mode to send real
