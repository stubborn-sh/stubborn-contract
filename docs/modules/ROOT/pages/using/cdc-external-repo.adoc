...
locally through the Stubborn Contract Plugin (SCC Plugin).
...
You also need consumer code that has Stubborn Contract Stub Runner set up.
For an example of such a project, see {samples_code}/consumer[this sample].
You also need producer code that has Stubborn Contract set up, together with a plugin.
...
$ ./mvnw clean install
----
. Sets up Stubborn Contract (SCC) Stub Runner in the consumer tests, to:
