...
Spring Boot 4.x split modules (`spring-boot-restclient`, `spring-boot-http-client`)
so you do not need to declare those versions separately.

Next, add the `Stubborn Contract Verifier` Maven plugin, as follows:
...
The `stubborn-dependencies` BOM manages `io.rest-assured:rest-assured:6.0.0` so you
do not need to specify the version in your project. Add the dependency without a
`<version>`:
...
By default, Stubborn Contract Verifier looks for stubs in the
...
Given that structure, Stubborn Contract Verifier creates a test class named
...
====
mvn sh.stubborn:stubborn-maven-plugin:run \
...
`sh.stubborn.contract.verifier.tests`.
...
* `contractsProperties`: A map that contains properties to be passed to Stubborn Contract
...
When using Stubborn Contract Verifier in the default (`MockMvc`), you need to create a base
...
If your base classes differ between contracts, you can tell the Stubborn Contract
...
`packageWithBaseClasses` property to `com.example.base`, then Stubborn Contract
...
[source,xml,indent=0]
----
include::{plugins_path}/stubborn-maven-plugin/src/test/projects/basic-generated-baseclass/pom.xml[tags=convention,indent=0]
----
...
[source,xml,indent=0]
----
include::{plugins_path}/stubborn-maven-plugin/src/test/projects/basic-baseclass-from-mappings/pom.xml[tags=mapping,indent=0]
----
...
The Stubborn Contract Maven Plugin generates verification code in a directory called
