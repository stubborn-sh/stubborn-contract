...
WireMock lets you register custom extensions. By default, Stubborn Contract registers
...
`sh.stubborn.contract.verifier.dsl.wiremock.WireMockExtensions` interface.
...
will be a valid WireMock's `StubMapping` JSON / map or an actual `StubMapping` object, Stubborn Contract will patch the generated
...
`sh.stubborn.contract.verifier.converter.StubProcessor` key register your own implementation of a stub processor. For your convenience we've created an interface called `sh.stubborn.contract.verifier.wiremock.WireMockStubPostProcessor` that is dedicated to WireMock.

You'll have to implement methods to inform Stubborn Contract whether the post processor is applicable for a given contract and how should the post processing look like.
