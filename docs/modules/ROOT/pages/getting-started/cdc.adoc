...
The server-side code is available under {samples_code}[Stubborn Contract Samples] repository `samples/standalone/dsl/http-server` path, and the client-side code is available under Stubborn Contract's repository `samples/standalone/dsl/http-client` path.
...
. Add the Stubborn Contract (SCC) plugin.
...
First, we add the `Stubborn Contract` BOM, as the following example shows:

{samples_code}/standalone/dsl/http-server/pom.xml[Click here to see the code]

Next, add the `Stubborn Contract Verifier` Maven plugin, as the following example shows:

{samples_code}/standalone/dsl/http-server/pom.xml[Click here to see the code]

Since the plugin was added, you get the `Stubborn Contract Verifier` features, which,
...
In order to profit from the Stubborn Contract Stub Runner functionality of automatic
stub downloading, you must do the following in your consumer side project (`Loan
Application service`):

. Add the `Stubborn Contract` BOM, as follows:
+
{samples_code}/standalone/dsl/http-client/pom.xml[Click here to see the code]

. Add the dependency to `Stubborn Contract Stub Runner`, as follows:
+
{samples_code}/standalone/dsl/http-client/pom.xml[Click here to see the code]
...
Now you can disable the offline work for Stubborn Contract Stub Runner and indicate
