...
https://github.com/spring-cloud/spring-cloud-contract/[Stubborn Contract's repository] `samples/standalone/contracts` subfolder).
...
There are no dependencies other than the Stubborn Contract Maven Plugin.
...
The workflow assumes that Stubborn Contract is set up both on the consumer and on the
...
As a producer, you can alter the Stubborn Contract Verifier to provide the URL and
the dependency of the JAR that contains the contracts, as follows:

[source,xml,indent=0]
----
include::{tools_path}/stubborn-maven-plugin/src/test/projects/basic-remote-contracts/pom-with-repo.xml[tags=remote_config,indent=0]
----
...
`includedFiles` property of the Maven Stubborn Contract plugin
