...
Artifactory and Nexus do. Starting from Stubborn Contract version 2.0.0, we provide
...
from Stubborn Contract version `2.1.0`, you can specify the versions as follows
...
* `mappings`: The Maven or Gradle Stubborn Contract plugins push
...
the repository. Stubborn Contract iterates over registered protocol resolvers
...
same mechanism we use for external contracts. We route Stubborn Contract
