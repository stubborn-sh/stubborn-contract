...
https://grpc.io/[GRPC] is an RPC framework built on top of HTTP/2 for which Stubborn Contract has basic support.

IMPORTANT: Stubborn Contract has an experimental support for basic use cases of GRPC. Unfortunately, due to GRPC's tweaking of the HTTP/2 Header frames, it's impossible to assert the `grpc-status` header.
...
The base class would set up the application running on a random port. It will also set the `HttpVerifier` implementation to one that can use the HTTP/2 protocol. Stubborn Contract comes with the `OkHttpHttpVerifier` implementation.
