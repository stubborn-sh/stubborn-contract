...
generate WireMock stubs by using Stubborn Contract WireMock. To do so, write your
...
----
"API Producer"->"API Producer": Add Stubborn Contract (SCC) \nStub Runner dependency
...
[source,java,indent=0]
----
import static sh.stubborn.contract.wiremock.restdocs.WireMockRestDocs.verify;
...
NOTE:  In Stubborn Contract 5.0.x support for Rest Assured in Spring Rest Docs was removed due to https://github.com/spring-projects/spring-restdocs/issues/1000[Rest Assured being incompatible with Groovy 5].

[[features-rest-docs-contracts]]
== Generating Contracts with REST Docs

You can also generate Stubborn Contract DSL files and documentation with Spring REST
