...
The Stubborn Contract lets you provide your own, custom, implementation of the
`sh.stubborn.contract.verifier.http.HttpVerifier`. That way, you can use any client you want to send and receive a request. The default implementation in Stubborn Contract is `OkHttpHttpVerifier` and it uses OkHttp3 http client.
...
import sh.stubborn.contract.verifier.http.Request;
import sh.stubborn.contract.verifier.http.Response;
...
import static sh.stubborn.contract.verifier.http.Request.given;
