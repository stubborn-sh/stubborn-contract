...
To use Stubborn Contract Verifier with WireMock, you must use either a
...
dependencyManagement {
	imports {
		mavenBom "org.springframework.cloud:stubborn-dependencies:$\{GAVerifierVersion}"
	}
}
...
dependencyManagement {
	imports {
		mavenBom "org.springframework.cloud:stubborn-dependencies:$\{verifierVersion}"
	}
}
...
buildscript {
	...
	dependencies {
		...
		classpath "org.springframework.cloud:stubborn-gradle-plugin:$\{verifier_version}"
...
dependencyManagement {
	imports {
		mavenBom "org.springframework.cloud:stubborn-dependencies:$\{verifier_version}"
	}
}
...
is deprecated as of Stubborn Contract 3.0.0.
...
definition is treated as a single test. Stubborn Contract Verifier assumes that it
...
Given the preceding structure, Stubborn Contract Verifier creates a test class named
...
When using Stubborn Contract Verifier in MockMvc (the default), you need to create a base
...
[source,groovy,indent=0]
----
include::{plugins_path}/stubborn-gradle-plugin/src/test/resources/functionalTest/bootSimple/src/test/groovy/org/springframework/cloud/contract/verifier/twitter/places/BaseMockMvcSpec.groovy[tags=base_class,indent=0]
----
...
In a consuming service, you need to configure the Stubborn Contract Verifier plugin
