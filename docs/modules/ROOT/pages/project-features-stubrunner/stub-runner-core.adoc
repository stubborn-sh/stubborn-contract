...
your implementation of the `sh.stubborn.contract.stubrunner.HttpServerStubConfigurer`
interface. The implementations can alter
the configuration files for the given HTTP server stub.

Stubborn Contract Stub Runner comes with an implementation that you
can extend for WireMock:
`sh.stubborn.contract.stubrunner.provider.wiremock.WireMockHttpServerStubConfigurer`.
