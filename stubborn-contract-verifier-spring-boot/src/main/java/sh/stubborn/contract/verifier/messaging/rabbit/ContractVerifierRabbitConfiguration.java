...
import sh.stubborn.messaging.rabbit.StubbornRabbitMessageVerifier;
...
/**
 * Spring-on-top wiring for the Spring-free RabbitMQ
 * {@link StubbornRabbitMessageVerifier}. It exposes the verifier as the messaging
 ...
 * the same test setup used with Spring AMQP keeps working — only the underlying verifier
 * is now the Spring-free, plain {@code amqp-client} implementation. The verifier reuses
 * the fully configured underlying {@code com.rabbitmq.client.ConnectionFactory} (host,
 * port, credentials, virtual host) so no connection detail is lost.
 *
 * @author Marcin Grzejszczak
 */
@Configuration(proxyBeanMethods = false)
@ConditionalOnClass({ RabbitTemplate.class, StubbornRabbitMessageVerifier.class })
...
public class ContractVerifierRabbitConfiguration {
	...
	StubbornRabbitMessageVerifier stubbornRabbitMessageVerifier(ConnectionFactory connectionFactory) {
		return new StubbornRabbitMessageVerifier(rabbitConnectionFactory(connectionFactory));
	}

	@Bean
	@ConditionalOnMissingBean(ContractVerifierMessaging.class)
	ContractVerifierMessaging<RabbitMessage> contractVerifierRabbitMessaging(StubbornRabbitMessageVerifier verifier) {
