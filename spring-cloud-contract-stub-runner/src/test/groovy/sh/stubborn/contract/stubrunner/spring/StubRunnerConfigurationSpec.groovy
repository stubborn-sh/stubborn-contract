...
class StubRunnerConfigurationSpec {
	...
	void 'should start WireMock servers'() {
		...
		assert stubFinder.findStubUrl('sh.stubborn.contract.verifier.stubs', 'loanIssuance') != null
		assert stubFinder.findStubUrl('loanIssuance') != null
		assert stubFinder.findStubUrl('loanIssuance') == stubFinder.findStubUrl('sh.stubborn.contract.verifier.stubs', 'loanIssuance')
		assert stubFinder.findStubUrl('loanIssuance') == stubFinder.findStubUrl('sh.stubborn.contract.verifier.stubs:loanIssuance')
		assert stubFinder.findStubUrl('sh.stubborn.contract.verifier.stubs:loanIssuance:0.0.1-SNAPSHOT') == stubFinder.findStubUrl('sh.stubborn.contract.verifier.stubs:loanIssuance:0.0.1-SNAPSHOT:stubs')
		assert stubFinder.findStubUrl('sh.stubborn.contract.verifier.stubs:fraudDetectionServer') != null
		and:
		assert stubFinder.findAllRunningStubs().isPresent('loanIssuance')
		assert stubFinder.findAllRunningStubs().isPresent('sh.stubborn.contract.verifier.stubs', 'fraudDetectionServer')
		assert stubFinder.findAllRunningStubs().isPresent('sh.stubborn.contract.verifier.stubs:fraudDetectionServer')
	...
	void 'should register started servers as environment variables'() {
		...
		and:
		assert environment.getProperty("spring.cloud.contract.stubrunner.runningstubs.fraudDetectionServer.port") != null
		assert stubFinder.findAllRunningStubs().getPort("fraudDetectionServer") == (environment.getProperty("spring.cloud.contract.stubrunner.runningstubs.sh.stubborn.contract.verifier.stubs.fraudDetectionServer.port") as Integer)
	}
