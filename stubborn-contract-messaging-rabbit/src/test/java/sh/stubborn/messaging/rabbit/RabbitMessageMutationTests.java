...
/**
 * Mutation-focused unit tests for the pure {@link RabbitMessage} carrier.
 *
 * @author Marcin Grzejszczak
 */
@SuppressWarnings("NullAway")
class RabbitMessageMutationTests {
	...
	void nullPayloadIsPreserved() {
		RabbitMessage message = new RabbitMessage(null, new LinkedHashMap<>());
		assertThat(message.getPayload()).isNull();
	}

	@Test
	void nullHeadersBecomeEmptyMutableMap() {
		RabbitMessage message = new RabbitMessage("body", null);
	...
	void toStringContainsPayloadAndHeaders() {
		...
		headers.put("h1", "v1");
		RabbitMessage message = new RabbitMessage("body", headers);
		assertThat(message.toString()).isEqualTo("RabbitMessage{payload=body, headers={h1=v1}}");
	}
