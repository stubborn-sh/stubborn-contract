...
class JsonAsserter implements JsonVerifiable {
	...
	public JsonVerifiable contains(final Object value) {
		...
		// this is fake part of jsonpath since in the next section we will remove this
		// entry
	...
	public JsonVerifiable isEqualTo(String value) {
		...
		ReadyToCheckAsserter readyToCheck = new ReadyToCheckAsserter(parsedJson, jsonPathBuffer, fieldName,
				jsonAsserterConfiguration);
		readyToCheck.jsonPathBuffer.removeLast();
		readyToCheck.jsonPathBuffer
			.offer("[?(@." + String.valueOf(fieldName) + " == " + wrapValueWithSingleQuotes(value) + ")]");
	...
	public JsonVerifiable isInstanceOf(Class clazz) {
		...
		throw new IllegalStateException("For JSON path [" + readyToCheck.jsonPath() + "] instance of ["
				+ object.getClass().getSimpleName() + "] is not assignable from [" + clazz.getSimpleName() + "]");
	}
	...
	public JsonVerifiable matches(String value) {
		...
		ReadyToCheckAsserter readyToCheck = new ReadyToCheckAsserter(parsedJson, jsonPathBuffer, fieldName,
				jsonAsserterConfiguration);
		readyToCheck.jsonPathBuffer.removeLast();
		readyToCheck.jsonPathBuffer.offer(
				"[?(@." + String.valueOf(fieldName) + " =~ /" + stringWithEscapedSingleQuotesForRegex(value) + "/)]");
		updateCurrentBuffer(readyToCheck);
		readyToCheck.checkBufferedJsonPathString();
		return readyToCheck;
	}
	...
	public JsonVerifiable isEmpty() throws IllegalStateException {
		...
		if (!isObjectEmpty(o)) {
			throw new IllegalStateException("Parsed JSON [" + parsedJson.jsonString() + "] with the JSON path ["
					+ jsonPathString + "] is not empty!");
		}
		return this;
	}
	...
	public JsonVerifiable value() {
		ReadyToCheckAsserter readyToCheckAsserter = new ReadyToCheckAsserter(parsedJson, jsonPathBuffer, fieldName,
				jsonAsserterConfiguration);
	...
	private @Nullable JSONArray check(String jsonPathString) {
		...
		if (array.isEmpty() || containsEmptyElementsOnly(array)) {
			throw new IllegalStateException("Parsed JSON [" + parsedJson.jsonString()
					+ "] doesn't match the JSON path [" + jsonPathString + "]");
		}
		return array;
	}
	...
	public JsonVerifiable hasSize(int size) {
		...
		if (array == null || array.size() != size) {
			throw new IllegalStateException("Parsed JSON <" + parsedJson.jsonString() + "> doesn't have the " + "size <"
					+ size + "> for JSON path <" + createJsonPathString() + ">. The size is <"
	...
	private void logOverridingWarning() {
		log.trace(
				"WARNING!!! Overriding verification of the JSON Path. Your tests may pass even though they shouldn't");
	}
	...
	protected String wrapValueWithSingleQuotes(Object value) {
		return value instanceof String ? "'" + stringWithEscapedSingleQuotes(value) + "'" : value.toString();
	}
