...
@AutoConfigureStubRunner(ids = "sh.stubborn.contract.verifier.stubs:producerWithMultipleConsumers",
		repositoryRoot = "classpath:m2repo/repository/", stubsMode = StubRunnerProperties.StubsMode.REMOTE,
		stubsPerConsumer = true)
@ActiveProfiles("streamconsumer")
class StubRunnerStubsPerConsumerNotMatchingConsumerTests {
	...
	void shouldFailToStartWhenJarIsFoundButThereAreNoStubsOrContracts() {
		this.contextRunner.run((context) -> {
			...
			assertThat(context.getStartupFailure()).hasMessageContaining(
					"No stubs or contracts were found for [sh.stubborn.contract.verifier.stubs:producerWithMultipleConsumers:0.0.1-SNAPSHOT:stubs] and the switch to fail on no stubs was set");
		});
	}

	@Configuration
	@EnableAutoConfiguration(exclude = { ZookeeperAutoConfiguration.class })
