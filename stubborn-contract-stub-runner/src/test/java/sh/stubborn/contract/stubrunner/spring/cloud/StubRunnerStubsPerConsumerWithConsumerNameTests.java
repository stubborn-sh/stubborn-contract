...
class StubRunnerStubsPerConsumerWithConsumerNameTests {
	...
	void shouldStartHttpStubServersForFooConsumerOnly() throws Exception {
		...
		ResponseEntity<String> notFound = template.getForEntity(stubUrl + "/bar-consumer", String.class);
	...
	void shouldTriggerAMessageByLabelFromProperConsumer() {
		...
		assertThat(receivedMessage.getPayload()).isEqualTo("{\"bookName\":\"foo_for_foo\"}".getBytes());
		assertThat(receivedMessage.getHeaders().get("BOOK-NAME")).isEqualTo("foo_for_foo");
	}
