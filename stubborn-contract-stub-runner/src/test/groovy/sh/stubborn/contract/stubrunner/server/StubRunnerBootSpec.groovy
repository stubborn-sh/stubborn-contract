...
class StubRunnerBootSpec {
	...
	void 'should return a list of running stub servers in "full ivy port" notation'() {
		...
		then:
			...
			assert root.'sh.stubborn.contract.verifier.stubs:bootService:0.0.1-SNAPSHOT:stubs' instanceof Integer
	}

	@Test
	void 'should return a port on which a #stubId stub is running'() {
		given:
		def stubIds = ['sh.stubborn.contract.verifier.stubs:bootService:+:stubs',
				   'sh.stubborn.contract.verifier.stubs:bootService:0.0.1-SNAPSHOT:stubs',
				   'sh.stubborn.contract.verifier.stubs:bootService:+',
				   'sh.stubborn.contract.verifier.stubs:bootService',
				   'bootService']
		stubIds.each {
			when:
	...
	void 'should return a list of messaging labels that can be triggered when version and classifier are passed'() {
		...
		then:
			def root = new JsonSlurper().parseText(response)
			assert root.'sh.stubborn.contract.verifier.stubs:bootService:0.0.1-SNAPSHOT:stubs'?.containsAll(["return_book_1"])
	}
	...
	void 'should trigger a messaging label for a stub with #stubId ivy notation'() {
		given:
			...
			RestAssuredMockMvc.standaloneSetup(new HttpStubsController(stubRunning), new TriggerController(stubRunning))
		and:
			def stubIds = ['sh.stubborn.contract.verifier.stubs:bootService:stubs', 'sh.stubborn.contract.verifier.stubs:bootService', 'bootService']
		stubIds.each {
			when:
	...
	void 'should throw exception when trigger is missing'() {
		...
		.hasMessageContaining("sh.stubborn.contract.verifier.stubs:bootService:0.0.1-SNAPSHOT:stubs=")
	}
