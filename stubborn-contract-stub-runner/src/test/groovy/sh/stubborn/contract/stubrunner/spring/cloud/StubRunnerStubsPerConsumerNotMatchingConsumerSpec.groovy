...
class StubRunnerStubsPerConsumerNotMatchingConsumerSpec {
	...
	private ApplicationContextRunner contextRunner = new ApplicationContextRunner()
			...
			.withPropertyValues("spring.application.name=bar-not-matching-consumer",
					"spring.cloud.contract.stubrunner.jms.enabled=false", "spring.profiles.active=streamconsumer", "spring.cloud.discovery.enabled=false", "spring.cloud.service-registry.auto-registration.enabled=false", "spring.cloud.contract.stubrunner.ids=sh.stubborn.contract.verifier.stubs:producerWithMultipleConsumers", "spring.cloud.contract.stubrunner.repositoryRoot=classpath:m2repo/repository/", "spring.cloud.contract.stubrunner.stubsMode=REMOTE", "spring.cloud.contract.stubrunner.stubsPerConsumer=true");
	...
	void 'should fail to start when JAR is found but there are no stubs or contracts'() {
		expect:
			this.contextRunner.run((context) -> {
				...
				assertThat(context.getStartupFailure()).hasMessageContaining("No stubs or contracts were found for [sh.stubborn.contract.verifier.stubs:producerWithMultipleConsumers:0.0.1-SNAPSHOT:stubs] and the switch to fail on no stubs was set")
			});
	}
