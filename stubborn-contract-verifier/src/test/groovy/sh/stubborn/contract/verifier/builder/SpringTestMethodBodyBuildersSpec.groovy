...
World.'''"""
	...
	def 'should generate proper test code when having multipart parameters with #methodBuilderName'() {
		given:
			// tag::multipartdsl[]
			sh.stubborn.contract.spec.Contract contractDsl = sh.stubborn.contract.spec.Contract.make {
				request {
					method 'PUT'
					url '/multipart'
					headers {
...
'''
	...
	def 'should resolve Optional object when used in query parameters'() {
		...
		when:
			String test = singleTestGenerator(contractDsl)
		then:
			!test.contains('sh.stubborn.contract.spec.internal.OptionalProperty')
